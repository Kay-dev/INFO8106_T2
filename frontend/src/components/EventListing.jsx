...
const EventListing = ({ event, userEvents, handleSubscribe, handleUnsubscribe }) => {
  ...
  const buttonElement = () => {
    if (!user || (user.role === 'user' && !userEvents.includes(event.id))) {
      return (
        <button
    ...
    if (user.role === 'user' && userEvents.includes(event.id)) {
      return (
        <button
  ...
  return (
    <div className='bg-white rounded-xl shadow-md relative'>
      <div className='p-4' onClick={toEventPage}>
        ...
        <h5 className='text-indigo-500 mb-2'>{dayjs(event.startTime).format('YYYY-MM-DD HH:mm')}~&nbsp;{dayjs(event.endTime).format('YYYY-MM-DD HH:mm')}</h5>
